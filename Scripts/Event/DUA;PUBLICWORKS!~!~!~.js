...
//start replaced branch: EMSE:DUA_PWORKS
 {
docDetails = newDocModelArr[eachDoc];
//for (x in docDetails) comment(x + ' = ' + docDetails[x]);
if (matches(docDetails['docGroup'],'PW','PW ACA') && matches(docDetails['docCategory'],'Plans','Construction Plans') && matches(docDetails['categoryByAction'],'RESUBMIT')) {
...
//end replaced branch: EMSE:DUA_PWORKS;
